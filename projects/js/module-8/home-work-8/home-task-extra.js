...
class Gallery {
  constructor({ items, parentNode, defaultActiveItem }) {
    ...
    list.addEventListener('click', event => {
      ...
      if (bigImageSrc) {
        const fullviewImg = parentNode.querySelector('.fullview img');
        fullviewImg.setAttribute('src', bigImageSrc);
      
        const previews = parentNode.querySelectorAll('.preview li');
...
new Gallery({
  items: galleryItems,
  parentNode: document.querySelector('.image-gallery-2'),
  defaultActiveItem: 1,
});

new Gallery({
  items: galleryItems,
  parentNode: document.querySelector('.image-gallery-3'),
  defaultActiveItem: 3,
});
