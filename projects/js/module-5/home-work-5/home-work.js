 ...
 */
const getUsersByEyeColor = (arr, color) => arr.filter(ar => ar.eyeColor === color);
 ...
 */
const getUsersByGender = (arr, gender) => (arr.filter(ar => ar.gender === gender)).map(ar => ar.name);
 ...
 */
const getInactiveUsers = (arr)=> arr.filter(ar =>!ar.isActive);
 ...
 */
const getUserByEmail = (arr, email) => arr.find(ar => ar.email === email);
 ...
 */
const getUsersWithAge = (arr, min, max) => arr.filter(ar => ar.age >= min && ar.age <= max );
 ...
 */
const getTotalBalance = arr => arr.reduce((acc,arr)=> acc + arr.balance, 0);
 ...
 */
const getUsersByFriend = (arr, name) => (arr.filter(ar => ar.friends.includes(name))).map(ar => ar.name);
 ...
 */
const getAllUniqueSkills = arr => arr.reduce((acc, ar) => acc.concat(ar.skills.filter(skill => !acc.includes(skill))), []).sort();
 ...
 */
const getUserNamesSortedByFriendsCount = arr => arr.sort((a, b) => a.friends.length - b.friends.length).map(element => element.name);
